...
public final class CreateSlotReqDTO implements Serializable {
    ...
    @ApiModelProperty(name = "appIds", value = "应用ID", required = true)
    @NotNull(message = "应用ID不能为空")
    private List<Long> appIds;

    @ApiModelProperty(name = "name", value = "资源位名称", required = true)
    @NotNull(message = "资源位名称不能为空")
    @Length(max = 30, message = "资源位名称不得超过30个字符")
    private String name;

    @ApiModelProperty(name = "specId", value = "资源位规格ID", required = true)
    @NotNull(message = "规格ID不能为空")
    @Min(value = 100000000000000L, message = "资源位标识不合法")
    private Long specId;

    @ApiModelProperty(name = "type", value = "资源位类型(SPLASH|BULLET|BANNER|FEEDS)", required = true)
    @NotNull(message = "资源位类型不能为空")
    private SlotType type;

    @ApiModelProperty(name = "os", value = "支持系统类型(IOS|ANDROID|IOSROID|NA)", required = true)
