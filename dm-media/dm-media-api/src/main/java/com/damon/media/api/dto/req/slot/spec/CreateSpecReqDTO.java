...
public final class CreateSpecReqDTO implements Serializable {
    ...
    @ApiModelProperty(name = "slotType", value = "资源位类型(SPLASH|BULLET|BANNER|FEEDS)", required = true)
    @NotNull(message = "资源位类型不能为空")
    private SlotType slotType;

    @ApiModelProperty(name = "imageType", value = "图片类型(PNG,JPG,GIF)", required = true)
    @NotNull(message = "图片类型不能为空")
//    @Pattern(regexp = "^((PNG)|(JPG)|(GIF))+", message = "图片类型取值应为(PNG,JPG,GIF)之一种或多种")
    private String imageType;

    @ApiModelProperty(name = "imageSize", value = "图片大小(单位:K)", required = true)
    @NotNull(message = "图片大小不能为空")
    @Min(value = 0, message = "图片大小必须大于0")
    private Integer imageSize;

    @ApiModelProperty(name = "snapshot", value = "资源位规格示意图", required = true)
