...
public final class CreateAppReqDTO implements Serializable {
    ...
    @ApiModelProperty(value = "媒体归属(ASSET|ALIEN)", required = true)
    @NotNull(message = "媒体归属不能为空")
    private MediaSource source;


    @ApiModelProperty(value = "应用类型(APP|WAP|WEB|POS)", required = true)
    @NotNull(message = "应用类型不能为空")
    private MediaType type;


    @ApiModelProperty(value = "应用的操作系统平台(IOS|ANDROID)", required = true)
    @NotNull(message = "操作系统不能为空")
    private OSCategory os;


    @ApiModelProperty(value = "应用下载地址", required = true)
...
// String dddf ="^(https?://)?([\\da-z\\.-]+)\\.([a-z\\.]{2,6})([/\\w \\.-]*)*\\/?$/";
    ...
    @ApiModelProperty(value = "程序主包名称", required = true)
    ...
    @ApiModelProperty(value = "APP示意图", required = true)
    ...
    @ApiModelProperty(value = "应用人群兴趣关键词，空格分隔，每个关键词不超过10个字")
